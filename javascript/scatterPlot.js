...
export async function drawScatterPlot (promises, filterHandler) {
  promises.then(function ([world, co2, mappedNaturalDisasterData]) {
    ...
    var x = d3
      ...
      .domain([
        1,
        d3.max(filteredCo2, function (d) {
          return getEmissionsForCountry(d, year, emissionTypes)
        })
      ]) //den här skalan blir inte rätt
      .range([0, width])
    svg
      .append('g')
    ...
    var y = d3
      ...
      .domain([
        0,
        d3.max(filteredNatDis, function (d) {
          return d.nbr
        })
      ])
      .range([height, 0])
    ...
    if (emissionTypes.length > 0) {
      ...
      circ
        ...
        .data(filteredCo2)
        .attr('x', function (d) {
          return x(getEmissionsForCountry(d, year, emissionTypes))
        })
        .attr('width', 250)
        .attr('height', 70)
        .attr('rx', 5)
        ...
        .attr('fill', '#ffffff')
        .attr('id', function (d) {
          return 'rect' + d.iso_code.toString() + d.year.toString()
        })
        .style('opacity', '0')
      ...
      circ
        ...
        .append('text')
        .attr('y', function (d) {
          return parseInt(y(getNaturalDisastersForCountry(d, year)) + 20)
        })
        .data(filteredCo2)
        .attr('x', function (d) {
          if (getEmissionsForCountry(d, year, emissionTypes) > 0) {
            return parseInt(
              x(getEmissionsForCountry(d, year, emissionTypes)) + 10
            )
          } else {
            return 10
          }
        ...
        .attr('id', function (d) {
          return 'text' + d.iso_code.toString() + d.year.toString()
        })
        .style('opacity', '0')
        .text(function (d) {
          return d.country
        ...
        .text(function (d) {
          return (
            'Co2 Emissions: ' +
            getEmissionsForCountry(d, year, emissionTypes) +
            ' million ton'
          )
        })
        .append('tspan')
        ...
        .text(function (d) {
          return (
            'Natural Disasters: ' + getNaturalDisastersForCountry(d) + ' st'
          )
        })
        .attr('dy', 20)
...
function getEmissionsForCountry (row, year, emissionTypes) {
  ...
  if (emissionTypes.length > 0) {
    ...
    for (let y = 1700; y <= year; y++) {
      ...
      if (emissionTypes.includes('co2') && row.co2 && parseInt(row.co2) > 0) {
        tot = tot + parseInt(row.co2)
      } else {
        if (
          emissionTypes.includes('oil_co2') &&
          row.oil_co2 &&
          parseInt(row.oil_co2) > 0
        ) {
          tot = tot + parseInt(row.oil_co2)
        }
        if (
          emissionTypes.includes('gas_co2') &&
          row.gas_co2 &&
          parseInt(row.gas_co2) > 0
        ) {
          tot = tot + parseInt(row.gas_co2)
        }
        if (
          emissionTypes.includes('coal_co2') &&
          row.coal_co2 &&
          parseInt(row.coal_co2) > 0
        ) {
          tot = tot + parseInt(row.coal_co2)
        }
        if (
          emissionTypes.includes('cement_co2') &&
          row.cement_co2 &&
          parseInt(row.cement_co2) > 0
        ) {
